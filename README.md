# 대한예수교장로회 남사보배로운교회 홈페이지

React + Vite + TypeScript + Firebase 기반 교회 공식 홈페이지. 방문자 회원가입 없이 최고관리자
1명 + 부관리자 최대 3명이 사이트 전체를 인라인 편집하는 구조입니다.

- 로컬 참고 문서: `prd/` (gitignore — GitHub/Firebase 계정 정보 포함, 원격에 올리지 않음)

## 저장소 & 배포 정보

| 항목 | 값 |
|---|---|
| GitHub 저장소 | https://github.com/namsabo180708-prog/namsa-bbrc |
| 기본 브랜치 | `main` |
| GitHub 계정 | `namsabo180708-prog` (`namsabo180708@gmail.com`) |
| Firebase 프로젝트 ID | `namsa-bbrc` |
| Firebase 콘솔 | https://console.firebase.google.com/project/namsa-bbrc/overview |
| Hosting 배포 주소 | https://namsa-bbrc.web.app |
| 도메인 | https://www.bobaeroun.org |
...
- **Footer**: 2열 — 좌측(로고·연락처 + 저작권·Email·Admin) | 우측(Quick Link 뱃지 중앙 + 바로가기 6개). 하단 별도 바 없음. 링크 호버 언더바는 Topbar와 동일.
...
```bash
npm run build
npx firebase deploy --only hosting --project namsa-bbrc

# Rules / Storage까지 함께
npx firebase deploy --only hosting,firestore:rules,firestore:indexes,storage --project namsa-bbrc

# Functions — Blaze(종량제) 요금제 전환 필요
cd functions && npm install && npm run build && cd ..
npx firebase deploy --only functions --project namsa-bbrc
```
...
VITE_FIREBASE_AUTH_DOMAIN=namsa-bbrc.firebaseapp.com
VITE_FIREBASE_PROJECT_ID=namsa-bbrc
VITE_FIREBASE_STORAGE_BUCKET=namsa-bbrc.firebasestorage.app
VITE_FIREBASE_MESSAGING_SENDER_ID=562294292744
VITE_FIREBASE_APP_ID=
```

`apiKey` 등 위 값들은 Firebase 웹 SDK 설정으로 공개되어도 안전합니다(실제 보안은 Firestore/
Storage Rules가 담당). 반면 서비스 계정 JSON 키, `prd/GitHub-info.md`(PAT 등 로컬 참고 메모)는
절대 커밋/공유하지 않습니다.

## 공식 명칭

모든 노출 위치에서 **대한예수교장로회 남사보배로운교회** 사용.
