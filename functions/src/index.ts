 ...
 *
 * namsa-bbrc의 서비스 계정 키를 노회 프로젝트에 그대로 넘기지 않기 위한 브로커 —
 * 호출자가 진짜 namsa-bbrc 관리자인지 이 함수(=namsa-bbrc 소유 코드)만 판단하고,
 * 결과만 노회 쪽에 알려준다. namsa-bbrc의 Firestore/Storage 자체에는 노회 프로젝트가
 * 직접 접근하지 않는다.
 *
 * 사용법: `Authorization: Bearer <namsa-bbrc ID 토큰>` 헤더로 GET/POST 호출.
 * ID 토큰 검증이 실질적인 보안 경계라 CORS는 느슨하게(`*`) 열어둔다 — 노회 포털
 * 프론트엔드가 namsa-bbrc Firebase 설정(공개 값)으로 별도 로그인 후 이 엔드포인트를
