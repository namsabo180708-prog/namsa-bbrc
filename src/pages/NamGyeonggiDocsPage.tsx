...
export function NamGyeonggiDocsPage() {
  ...
  const startEdit = (target: PresbyteryDocument) => {
    // 버튼이 비활성화돼 있어도 방어적으로 한 번 더 막는다 — 노회가 보낸 수신 문서는 수정 불가
  ...
  const performDelete = async (target: PresbyteryDocument) => {
    // 버튼이 비활성화돼 있어도 방어적으로 한 번 더 막는다 — 노회가 보낸 수신 문서는 삭제 불가
    if (target.direction === 'inbound') return
    setDeleting(true)
    try {
                      ...
                      {editingId === d.id ? (
                        ...
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-8 w-8 p-0 disabled:text-paper-muted/40"
                            title={d.direction === 'inbound' ? '노회가 보낸 문서는 수정할 수 없습니다' : '수정'}
                            aria-label="수정"
                            disabled={d.direction === 'inbound'}
                            onClick={() => startEdit(d)}
                          >
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-8 w-8 p-0 disabled:text-paper-muted/40 enabled:hover:text-wine"
                            title={d.direction === 'inbound' ? '노회가 보낸 문서는 삭제할 수 없습니다' : '삭제'}
                            aria-label="삭제"
                            disabled={d.direction === 'inbound'}
